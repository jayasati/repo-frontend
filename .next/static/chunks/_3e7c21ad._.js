...
const useChatStore = (0, __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$zustand$2f$esm$2f$react$2e$mjs__$5b$app$2d$client$5d$__$28$ecmascript$29$__["create"])()((set, get)=>({
        ...
        initSession: (jobId, projectName, files)=>{
            ...
            if ((existing === null || existing === void 0 ? void 0 : existing.jobId) === jobId) return;
            set({
                session: {
                    jobId,
                    projectName,
 ...
 */ function buildContextFiles(result) {
    ...
    for (const name of moduleNames){
        ...
        if (moduleSmells.length > 0) {
            lines.push('Smells:');
            moduleSmells.forEach((s)=>lines.push("  - [".concat(s.severity, "] ").concat(s.type, ": ").concat(s.message)));
        }
        if (moduleHotspot) {
            lines.push("Hotspot: fan-out ".concat(moduleHotspot.fanOut, " (").concat(moduleHotspot.risk, " risk)"));
        }
        if (moduleCycles.length > 0) {
            lines.push('Involved in cycles:');
            moduleCycles.forEach((c)=>lines.push("  - ".concat(c.nodes.join(' → '))));
        }
